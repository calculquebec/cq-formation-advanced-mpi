...
program MatVec_MPI
  ...
  do repeat=1,10000
     ...
     ! Note: reduction on myv VECTOR, supported in OpenMP 3.0+ for Fortran only
!$OMP PARALLEL DO PRIVATE(i,j) REDUCTION(+:myv)
     do j=1,n
        do i=is,ie
...
!$OMP END PARALLEL DO
     ...
     call mpi_allgatherv(myv, counts(myid+1), MPI_DOUBLE_PRECISION, &
          v, counts, displs, MPI_DOUBLE_PRECISION, MPI_COMM_WORLD, ierr)
  enddo
